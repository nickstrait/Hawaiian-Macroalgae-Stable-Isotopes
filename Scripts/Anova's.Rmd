...
```{r}
#Anova's of best genera spatially distributed in shallow depths across the archipelago
summary(aov(N15 ~ Genus + Region + Location, data = spatial_data_shlw))
TukeyHSD(aov(N15 ~ Genus + Region + Location, data = spatial_data_shlw))
summary(aov(Percent ~ Genus + Region + Location, data = spatial_data_shlw))
TukeyHSD(aov(Percent ~ Genus + Region + Location, data = spatial_data_shlw))
```

```{r}
#Anova's of best genera spatially distributed in mesophotic depths across the archipelago
summary(aov(N15 ~ Genus + Region + Location, data = spatial_data_meso))
TukeyHSD(aov(N15 ~ Genus + Region + Location, data = spatial_data_meso))
summary(aov(Percent ~ Genus + Region + Location, data = spatial_data_meso))
TukeyHSD(aov(Percent ~ Genus + Region + Location, data = spatial_data_meso))
```
